...
\usepackage[margin=0.92in,top=0.85in]{geometry}
\usepackage[sc]{mathpazo}
\linespread{1.02}
\usepackage[T1]{fontenc}
\usepackage{booktabs,graphicx,amsmath,microtype,xcolor}
\definecolor{linkblue}{RGB}{26,45,95}
\usepackage[hidelinks,colorlinks=true,linkcolor=linkblue,urlcolor=linkblue,citecolor=linkblue]{hyperref}
\usepackage{titlesec}
\titlespacing*{\section}{0pt}{10pt}{4pt}
\titleformat{\section}{\large\scshape}{\thesection}{0.7em}{}
\usepackage[font=small,labelfont=bf,skip=6pt]{caption}
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\fancyhead[R]{\small\itshape Preprint \,$\cdot$\, August 2026}
\fancyfoot[C]{\small\thepage}
\setlength{\parskip}{3pt}
\begin{document}

\begin{center}
{\LARGE\scshape Does Chain-of-Thought Still Protect Against\\[3pt] J-Space Ablation When Problems Get Hard?}\\[14pt]
{\large Peter Flo}\\[3pt]
{\small Harvard University \,$\cdot$\, \href{https://pflo.org}{pflo.org}}\\[4pt]
{\small Abstract accepted at the New England Mechanistic Interpretability Workshop (NEMI 2026)}\\[6pt]
{\small Code and full records: \href{https://github.com/pgrindehollevik-harvard/jspace-4b}{\texttt{github.com/pgrindehollevik-harvard/jspace-4b}}}
\end{center}
...
\small
\textbf{Abstract.} Gurnee et al.\ (2026) show that ablating a model's J-space (the residual-stream
projections onto its most active Jacobian-lens directions) selectively impairs internal multi-step
reasoning while leaving ordinary next-token prediction largely intact, and that written
chain-of-thought protects against the ablation. We ask whether this workspace signature transfers
to small open models, via a pre-registered replication on Qwen3-4B gated on reproducing both of the
paper's signatures before the main experiment could run. The gate failed at eight settings across
two lenses, in an informative pattern: the J-lens directions are causally real at 4B (in the
lightest band, ablation costs 16 points on a reasoning control while exactly norm-matched random
ablation costs zero; $n{=}55$, exact McNemar $p{=}.012$), but they are not selective (34 to 42
percent of ordinary next-token predictions change). The workspace/automatic separation may be an
emergent property of scale, a hypothesis directly testable with the public lens suite (1.7B to 32B).
\end{minipage}
\end{center}
\vspace{6pt}
\section{Hypothesis (pre-registered before any experiment; tag \texttt{prereg-v1})}
Gurnee et al.~\cite{gurnee2026} show that a model's ``J-space'' can be deleted: at every token
...
that protection survives on a much smaller model, \texttt{Qwen/Qwen3-4B}~\cite{qwen3}, and whether
...
The companion repository~\cite{jlens} provides the lens but no ablation code, so we implemented the
...
token. The lens is the pre-fitted \texttt{neuronpedia/jacobian-lens} artifact~\cite{neuronpedia}, fit on
...
\texttt{math-verify}~\cite{mathverify} on the last \verb|\boxed{}|; no-answer, truncation, degeneration,
...
\begin{center}
\includegraphics[width=0.86\linewidth]{../results/figures/ladder.pdf}
\end{center}
...
\bibitem{gurnee2026} W.~Gurnee et al. Verbalizable Representations Form a Global Workspace in Language Models. \emph{Transformer Circuits Thread}, Anthropic, 2026. \url{https://transformer-circuits.pub/2026/workspace/index.html}
...
\bibitem{hendrycks2021} D.~Hendrycks et al. Measuring Mathematical Problem Solving with the MATH Dataset. arXiv:2103.03874, 2021.
\bibitem{lightman2023} H.~Lightman et al. Let's Verify Step by Step. arXiv:2305.20050, 2023. (Source of the MATH-500 subset, \texttt{HuggingFaceH4/MATH-500}.)
\bibitem{aime} AIME 2024 I/II and 2025 I/II via Hugging Face: \texttt{Maxwell-Jia/AIME\_2024}; \texttt{yentinglin/aime\_2025}.
\bibitem{jlens} Anthropic. \texttt{jacobian-lens}: companion code for the global workspace paper. \url{https://github.com/anthropics/jacobian-lens}, 2026.
\bibitem{neuronpedia} Neuronpedia. Pre-fitted Jacobian lenses for open models. \url{https://huggingface.co/neuronpedia/jacobian-lens}, 2026.
\bibitem{mathverify} H.~Kydl\'i\v{c}ek. Math-Verify. \url{https://github.com/huggingface/math-verify}, 2025.
\end{thebibliography}
