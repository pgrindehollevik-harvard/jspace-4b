...
*(Commands will be finalized as the pipeline lands; each step is resumable.)*

```bash
.venv/bin/python -m hw0.validate_lens      # lens-vs-logit-lens gate (order-ops eval)
.venv/bin/python -m hw0.calibrate          # band/strength calibration + positive-control stop-gate
.venv/bin/python -m hw0.run_grid           # main grid, interleaved, resumable JSONL
.venv/bin/python -m hw0.analyze            # grading, bootstrap CIs, trend model, figures
```

The principal tables and figures in the report are regenerated by `hw0.analyze` from
`results/*.jsonl`.
