...
restarts=0
until .venv/bin/python -u -m hw0.calibrate >> logs/calibrate_pen.log 2>&1; do
  code=$?
  restarts=$((restarts+1))
...
stalled=0
last_lines=-1
until .venv/bin/python -u -m hw0.run_grid --calibration results/calibration_pen.json >> logs/grid.log 2>&1; do
  code=$?
